...
## 2026-09-10 PPT P5 经济映射数字张冠李戴修复（保底）

- **问题**：P5「经济映射：BOM 成本 -10.26%，毛利率影响 -5.47%」数字张冠李戴。
  实际（`python app.py` 实跑 + 说明文档 §四.3）：**扭矩密度→渗透率/销量 -10.26%；重量→BOM 单位成本 -5.47%**。
  -10.26% 是销量（渗透率/份额）弹性，-5.47% 是重量→BOM 单位成本弹性，原 P5 把两条链的数字互换了。
- **修复**：`claim2value_pitch.tex` P5 一行改对 → 重编译 beamer pdf → 从修复版 PDF 渲染 P5 PNG 替换
  pptx 内 `image-5-1.png`（pptx 为 12 页全图式，仅换 P5）。
- **验证**：替换后 pptx 的 image-5-1.png 与修复版 PDF P5 页渲染逐像素一致（md5 相同）；
  修复版 PDF 文本层含「扭矩密度→渗透率/销量 -10.26%；重量→BOM 单位成本 -5.47%」。
- PPT 后续如改为网页端 AI 重制，以修复后口径为准。
